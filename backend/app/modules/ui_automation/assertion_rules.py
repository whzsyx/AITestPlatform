...
def _extract_expected_columns(expected: str) -> list[str]:
    ...
    if not match:
        return []
    # Phase 15.13: 先用更宽松的尾从句切分把整体性描述剔掉.
    # 旧模式只覆盖"顺序/位置/样式/显示/展示/括号及文字/无歧义"等关键词起头的从句,
    # 没覆盖"且这7列均位于「创建时间」列之前"这种"且/同时/并/以及/而后"
    # 等连接词起头的从句, 导致 _SPLIT_RE 用"，"再切一次时尾从句被当成第 N+1
    # 个"伪列名"进入比对 -> "表格列缺失：且这7列均位于..." 假阳性.
    # 现场参考: #60ec5996 case 2 step 2.
    raw = re.split(
        r"[，,；;。]\s*"
...
def _clean_expected_column_label(value: str) -> str:
    ...
    if re.search(r"括号|无歧义|完全一致|顺序完全一致|文字完全一致", cleaned):
        return ""
    # Phase 15.13: 第二道防线 -- 即便上游切分漏过, 这里识别"看起来是描述句而非列名"
    # 的内容直接丢. 真实列名罕见包含"位于/之前/之后/均/这\d+列"等位置/数量短语.
    # 兜底: 单元长度 > 12 且含 (且|均|这\d+列|位于|之前|之后|包含|完整|可见|对齐)
    # 这种"完整子句"特征词时丢弃. 列名极少超 12 字; 真要有, 也不会同时含子句词.
    if re.search(r"位于|之前|之后|这\d+列|^且|均位于", cleaned):
        return ""
    if len(cleaned) > 12 and re.search(r"且|均|包含|完整|可见|对齐|无遮挡|未截断", cleaned):
        return ""
    return cleaned
