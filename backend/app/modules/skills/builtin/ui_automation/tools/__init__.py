...
- ``list_testcase_modules``：泛化"跑用例"时先列模块供用户选择
...
九个 tool 的 OpenAI schema 与执行函数同时通过 ``ensure_ui_automation_tools_
...
def ui_automation_chat_openai_schemas() -> dict[str, dict]:
    """9 个 ``system__ui_automation__*`` tool 的 OpenAI Chat tool spec。"""
    return {
        LIST_TESTCASE_MODULES_TOOL_NAME: LIST_TESTCASE_MODULES_SCHEMA,
...
def ensure_ui_automation_tools_registered() -> None:
    ...
    register_tool(LIST_TESTCASE_MODULES_TOOL_NAME, exec_list_testcase_modules)
